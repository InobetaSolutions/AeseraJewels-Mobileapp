...
//   final InvestmentDetailController controller = Get.put(InvestmentDetailController());
  ...
  Widget _buildPurchasedList() {
    ...
    return Column(
      children: [
        ...
        if (hasCoinData) ...controller.coinPaymentHistory.map((coinPayment) {
          return _buildCoinPaymentCard(coinPayment, format);
        }),
        
        // Display Regular Catalog History
        if (hasCatalogData) ...controller.purchasedHistory.map((p) {
  ...
  Widget _buildCoinPaymentCard(CoinPayment coinPayment, NumberFormat format) {
    return _cardContainer([
      Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text("OrderID: ${coinPayment.id.substring(0, 8)}...",
              style: const TextStyle(
                  fontSize: 14, fontWeight: FontWeight.bold)),
                  
          Text(coinPayment.formattedCreatedAt,
              style: const TextStyle(
                  fontSize: 14, color: Color.fromARGB(255, 81, 81, 81))),
        ],
      ),
      ...
      _alignedRow("Total Grams", "${coinPayment.totalGrams.toStringAsFixed(2)} g"),
      ...
      _alignedRow("Payment Status", coinPayment.formattedStatus,
          valueWeight: FontWeight.w600, valueColor: coinPayment.statusColor),
    ]);
  }
      ...
      Row(
        ...
        children: [
          Text("TagID ${p.tagid}",
              style: const TextStyle(
                  fontSize: 16, fontWeight: FontWeight.bold)),
          Text(controller.formatDate(p.timestamp, pattern: 'dd-MMM-yyyy, hh:mm a'),
              style: const TextStyle(
                  fontSize: 14, color: Color.fromARGB(255, 81, 81, 81))),
        ],
      ),
